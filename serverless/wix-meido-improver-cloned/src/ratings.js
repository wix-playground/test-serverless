...
async function getAvgRatings(ctx) {
    ...
    const allRatings = flatRatings.reduce((avg, [dishId, rating]) => (Object.assign({}, avg, { [dishId]: [...(avg[dishId] || []), rating] })), {});
...
async function setRating(ctx, userId, dishId, rating) {
    ...
    await ctx.datastore.put(DATASTORE_KEY, Object.assign({}, ratings, { [userId]: Object.assign({}, userRatings, { [dishId]: rating }) }));
}
exports.setRating = setRating;
async function deleteRating(ctx, userId, dishId) {
    const ratings = await ctx.datastore.get(DATASTORE_KEY) || {};
    const userRatings = Object.assign({}, (ratings[userId] || {}));
    delete userRatings[dishId];
    await ctx.datastore.put(DATASTORE_KEY, Object.assign({}, ratings, { [userId]: userRatings }));
}
